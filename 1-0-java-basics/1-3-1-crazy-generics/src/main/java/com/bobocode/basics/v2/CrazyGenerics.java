...
public class CrazyGenerics {
     ...
     * that are {@link Serializable} and {@link Comparable}.
     *
     ...
     */
    interface ComparableCollection<E extends Comparable<? super E>> extends Collection<E>,
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean isValidCollection_T(Collection<T> entities,
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<T> comparator) {
            ...
            for (T element : elements) {
                if (result.isEmpty() && Objects.nonNull(element)) {
                    result = Optional.of(element);
                } else if (result.isPresent() && comparator.compare(element, result.get()) > 0) {
                    result = Optional.of(element);
                }
            }
            return result;
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            Optional<T> maxEntity = findMax((Iterable) entities, CREATED_ON_COMPARATOR);
            return maxEntity.orElseThrow();
        }
