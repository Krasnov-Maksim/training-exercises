    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            for (BaseEntity baseEntity : entities) {
                if (Objects.isNull(baseEntity.getUuid())) {
                    return true;
                }
            }
            return false;
         ...
         */
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<T> entities,
                T targetEntity) {
            int duplicates = 0;
            for (BaseEntity baseEntity : entities) {
                if (baseEntity.getUuid().equals(targetEntity.getUuid())) {
                    duplicates++;
                    if (duplicates > 1) {
                        return true;
                    }
